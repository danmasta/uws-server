...
export class uWSServer {
    ...
    listen (fn) {
        let { socket, app, uws, log, opts: { bind, port }} = this;
        if (!socket) {
    ...
    // Note: uWS sets content-length automatically for buffers
    async handleResponse (res, socket) {
        ...
        // Note: Regarding corking - Signal to uWS to "collect" writes for batching
        // into one syscall
