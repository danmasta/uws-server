...
import { METHODS_UWS as METHODS, SYMBOLS } from './constants.js';
    ...
    constructor (opts, fn, supp) {
        ...
        this.register(METHODS.any, '/*', async (socket, inc) => {
            ...
            try {
                ...
                log.error(new ServerError(err, 'Uncaught request error'));
                return this.clientError(err, socket);
            }
            try {
                res = await fetch(req, { req: inc, socket, server });
            } catch (err) {
                log.error(new ServerError(err, 'Uncaught fetch error'));
                return this.serverError(err, socket);
            }
            try {
                await this.respond(res, socket);
            } catch (err) {
                log.error(new ServerError(err, 'Uncaught response error'));
