...
import { HTTP_METHODS as METHODS, REGEX } from './constants.js';
...
export class ServeStatic {
    ...
    async getCached (path, stat, enc, start, end) {
        ...
        if (cache) {
            if (stat.size <= maxSize) {
                ...
                if (!buf) {
                    buf = await buffer(this.getStream(path, stat, enc, start, end));
                    cache.set(key, buf);
                }
                return buf;
            }
        }
        return this.getStream(path, stat, enc, start, end);
    }

    // Return a stream of file contents, optionally with compression
    getStream (path, stat, enc, start, end) {
    ...
    middleware () {
        ...
        return async (c, next) => {
            ...
            if (compress) {
                enc = this.getEnc(c.req.header('accept-encoding'));
            ...
            switch (c.req.method) {
                ...
                case METHODS.get:
                    ...
                    let ranges = this.getRanges(c.req.header('range'));
                    if (ranges && ranges[0] === 'bytes') {
                        ...
                        status = 206;
                        body = this.getStream(path, stat, enc, head, tail);
                    } else {
                        if (cache) {
                            body = await this.getCached(path, stat, enc);
                        } else {
                            body = this.getStream(path, stat, enc);
                        }
                    }
                    break;
                default:
